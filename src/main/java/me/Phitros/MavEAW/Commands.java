...
public class Commands implements CommandExecutor {
	//Check which players are allowed to set the factions territory, including which faction.
			...
			if (args.length < 1) {
				if (sender instanceof Player) {
					...
					if (player.hasPermission("eaw.commands.list"))
						player.sendMessage(ChatColor.AQUA + "EAW >> Avaible commands: \n " + ChatColor.YELLOW + "/eaw setteritory <faction name> or /eaw setteritory off");
                       return true;				
				}
				...
				case "setterritory":
					if (sender instanceof Player) {
							...
							if (args.length == 2) {			
							 if (args[1].equalsIgnoreCase("blue") || args[1].equalsIgnoreCase("green") || args[1].equalsIgnoreCase("yellow") || args[1].equalsIgnoreCase("red")) {
								createSelection.put(player, args[1].toLowerCase());
									player.sendMessage(ChatColor.AQUA + "EAW >> " + ChatColor.YELLOW + "Selection mode has been turned on for faction/Empire:" + args[1]);
							    	
									
								} else if (args[1].equalsIgnoreCase("off") && createSelection.containsKey(player)) {
									createSelection.remove(player);
									player.sendMessage(ChatColor.AQUA + "EAW >> " + ChatColor.YELLOW + "Selection mode has been turned off");
								} else {
									player.sendMessage(ChatColor.AQUA + "EAW >> " + ChatColor.YELLOW + "Valid form: \n /eaw setteritory <faction name> or \n /eaw setteritory off" );
								}
					  ...
					  sender.sendMessage(ChatColor.AQUA + "EAW >> " + ChatColor.RED + "The command: /eaw " + ChatColor.YELLOW +  args[0] + ChatColor.RED + " is an in-game command only!");
					  break;
